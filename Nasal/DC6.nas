...
setlistener("autopilot/internal/vert-speed-fpm", func (fpm) {
    ...
    if (kt > 180 and fpm.getValue() < 100 and fpm.getValue() > -100) {
        if (getprop("engines/engine[0]/misture") != 0.6) {
            setprop("engines/engine[0]/misture", 0.6);
            setprop("engines/engine[1]/misture", 0.6);
            setprop("engines/engine[2]/misture", 0.6);
            setprop("engines/engine[3]/misture", 0.6);
            print("Mistura ajustada em todos os motores para ", getprop("engines/engine[3]/misture"));
        }
    } else {
        if (getprop("engines/engine[0]/misture") == 0.6) {
            setprop("engines/engine[0]/misture", 1);
            setprop("engines/engine[1]/misture", 1);
            setprop("engines/engine[2]/misture", 1);
            setprop("engines/engine[3]/misture", 1);
            print("Mistura ajustada em todos os motores para ", getprop("engines/engine[3]/misture"));
        }
    }
});
